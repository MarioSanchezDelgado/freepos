...
{ 
  short (*fn)(struct keyverify *,_TCHAR *,short);
} VERIFY_KEY;

typedef struct keydisp
{
  short (*fn)(struct keydisp *,_TCHAR *);  /* ANY display function          */
} INPUT_DISPLAY;
...
extern short inp_get_data(INPUT_CONTROLLER *, _TCHAR *);
...
extern int vfy_preamble(_TCHAR);
...
extern short cdsp_write_string(_TCHAR *, short, short);
extern short cdsp_write_abs_string(_TCHAR *);
...
typedef struct tdisp1 {
  short (*fn)(struct tdisp1*,_TCHAR *);  /* one of the display functions      */
  _TCHAR *format;                       /* for example "_____.__"            */
  _TCHAR *cover;                        /* characters of above to cover: "_" */
  short window;                        /* window to display it in           */
  short row,col;                       /* display position of first '_'     */
} TEMPLATE_DISPLAY1;

typedef struct tdisp2 {
  short (*fn)(struct tdisp2*,_TCHAR *);  /* one of the display functions      */
  _TCHAR *format;                       /* for example "_____.__"            */
  _TCHAR *cover;                        /* characters of above to cover: "_" */
  short  window;                       /* window to display it in           */
  short  row,col;                      /* display position of first '_'     */
  _TCHAR password_char;                /* Character to show instead of key  */
} TEMPLATE_PASSWORD;

extern  void (*fn_inp_idle)(void);     /* External hookup variable      */

extern  short r2l_display(TEMPLATE_DISPLAY1 *display,_TCHAR *data);
extern  short l2r_display(TEMPLATE_DISPLAY1 *display,_TCHAR *data);
extern  short l2r_password(TEMPLATE_PASSWORD *display,_TCHAR *data);
extern  short r2l_password(TEMPLATE_PASSWORD *display,_TCHAR *data);
extern  void  format_display(TEMPLATE_DISPLAY1 *, _TCHAR *);
extern  void  format_display_passwrd(TEMPLATE_PASSWORD *, _TCHAR *);
