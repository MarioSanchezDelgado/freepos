/*
 *     Module Name       : WPOS_MN.C
 *
 *     Type              : Main entry of the WinPOS Application
 *                         
 *
 ...
 * --------------------------------------------------------------------------
 * 04-Oct-2000 Opening of print files moved to write.c                 J.D.M.
 * --------------------------------------------------------------------------
 * 09-Oct-2000 Changed prn_on into prn_on[printer]                     J.D.M.
 * --------------------------------------------------------------------------
 * 11-Dec-2000 Modified de_init(). Added registration of callback      R.N.B.
 *             functions for PWDN_TYPE and CART_TYPE
 * --------------------------------------------------------------------------
 * 17-Apr-2001 Changed chk_status, because when there is an error on
 *             init_environment_records the POS should still be able
 *             to receive new database records from backoffice to
 *             correct the problem.                                    J.D.M.
 * --------------------------------------------------------------------------
 * 06-Jun-2001 Added err_init_environment to prevent the cashier from  R.N.B.
 *             logging on when there is a database problem.
 * --------------------------------------------------------------------------
 * 05-Dec-2001 Added Article Finder.                                     M.W.
 * --------------------------------------------------------------------------
 * 24-Apr-2002 Implemented use of Version_mgr instead of linkdate      J.D.M.
 * --------------------------------------------------------------------------
 * 18-Mar-2003 Messageboxes are now placed ALWAYS in the foreground!   J.D.M.
 * --------------------------------------------------------------------------
 * 06-May-2004 Added vat amounts on total section.                       M.W.
 * --------------------------------------------------------------------------
 * 14-Oct-2004 Added call back function to put multisam actions
 *             in memory for faster performance.                       J.D.M.
 * --------------------------------------------------------------------------
 ...
 * --------------------------------------------------------------------------
 * 01-May-2007 Added till entity and callback function
 *             update_till_callback()                                  J.D.M.
 * --------------------------------------------------------------------------
 * 12-Jul-2011 Change of title in program                              A.C.M.
 * --------------------------------------------------------------------------
...
static _TCHAR ident[] = _T("@(#) Copyright Makro International AG,\n Aspermonstrasse 24,\n 7006 CHUR,\n Switzerland\n");
...
_TCHAR  sinal[4] = _T("   ");           /* 3 characters calculator           */
...
static void chk_status (_TCHAR *, short, short, short, short);
static void PrintMsg   (_TCHAR *);
...
//12-Jul-2011 acm - {
//==================================================================================================
_TCHAR *replace_str(_TCHAR *str, _TCHAR *orig, _TCHAR *rep)
{
  static _TCHAR buffer[4096];
  _TCHAR  *p;
...
_TCHAR * window_gettext(_TCHAR * buffer, HWND hwnd)
{
	int length;
...
void SetTitleApp(_TCHAR * app_new_title, char * app_old_title) //"Makro Perú"
{
	HWND hwnd;
	double  t1;
	double t2;
...
int WINAPI _tWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow )
{
  ...
  SetTitleApp("Point Of Sale - Makro Perú"," Point Of Sale - Makro Colombia");// //28-Jun-2011 acm - add
  ...
  ReadEnvironmentValue(TREE_SCRN_SETTINGS, _T("ShowCursor"), Val, sizeof(Val)/sizeof(_TCHAR)-1);
...
short pos_main(void)
{
  ...
  for(i=DEV_PRINTER1; i < NUMBER_OF_PRINTERS; i++) {
    ...
    ReadEnvironmentValue(TREE_TILL_DEPENDENT, dummy, Val, sizeof(Val)/sizeof(_TCHAR)-1);
    prn_on[i] = YES;
  ...
  print_to_file = NO;
  ReadEnvironmentValue(TREE_TILL_DEPENDENT, _T("PRINT_TO_FILE"), Val, sizeof(Val)/sizeof(_TCHAR)-1);
  ...
  ReadEnvironmentValue(TREE_TILL_DEPENDENT, _T("NO_PRINTER_NAGGING"), Val, sizeof(Val)/sizeof(_TCHAR)-1);
...
/*---------------------------------------------------------------------------*/
/*                             de_init                                       */
/* - main deinit function of WinPOS. All fatal life functions will be        */
/*   disabled.                                                               */
/*---------------------------------------------------------------------------*/
void de_init(_TCHAR *msg)
{
  short i;
  ...
  WriteLn(_T("Please wait while de-initialising..."));
...
/*                          chk_status                                       */
/*---------------------------------------------------------------------------*/
void chk_status(_TCHAR *name, short status, short status_success,
